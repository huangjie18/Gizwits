...
static int8_t ICACHE_FLASH_ATTR gizDataPoint2Event(gizwitsIssued_t *issuedData, eventInfo_t *info, dataPoint_t *dataPoints)
{
    ...
    if(0x01 == issuedData->attrFlags.flagLEDonoff)
    {
        info->event[info->num] = EVENT_LEDonoff;
        info->num++;
        dataPoints->valueLEDonoff = gizStandardDecompressionValue(LEDonoff_BYTEOFFSET,LEDonoff_BITOFFSET,LEDonoff_LEN,(uint8_t *)&issuedData->attrVals.wBitBuf,sizeof(issuedData->attrVals.wBitBuf));
    }
...
static int8_t ICACHE_FLASH_ATTR gizCheckReport(dataPoint_t *cur, dataPoint_t *last)
{
    ...
    if(last->valueLEDonoff != cur->valueLEDonoff)
    {
        GIZWITS_LOG("valueLEDonoff Changed\n");
        ret = 1;
    }
...
static int8_t ICACHE_FLASH_ATTR gizDataPoints2ReportData(dataPoint_t *dataPoints , devStatus_t *devStatusPtr)
{
    ...
    gizStandardCompressValue(LEDonoff_BYTEOFFSET,LEDonoff_BITOFFSET,LEDonoff_LEN,(uint8_t *)devStatusPtr,dataPoints->valueLEDonoff);

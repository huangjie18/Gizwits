...
static int8_t ICACHE_FLASH_ATTR gizDataPoint2Event(gizwitsIssued_t *issuedData, eventInfo_t *info, dataPoint_t *dataPoints)
{
    ...
    if(0x01 == issuedData->attrFlags.flagLED0on_off)
    {
        info->event[info->num] = EVENT_LED0on_off;
        info->num++;
        dataPoints->valueLED0on_off = gizStandardDecompressionValue(LED0on_off_BYTEOFFSET,LED0on_off_BITOFFSET,LED0on_off_LEN,(uint8_t *)&issuedData->attrVals.wBitBuf,sizeof(issuedData->attrVals.wBitBuf));
    }
        
    if(0x01 == issuedData->attrFlags.flagLED1on_off)
    {
        info->event[info->num] = EVENT_LED1on_off;
        info->num++;
        dataPoints->valueLED1on_off = gizStandardDecompressionValue(LED1on_off_BYTEOFFSET,LED1on_off_BITOFFSET,LED1on_off_LEN,(uint8_t *)&issuedData->attrVals.wBitBuf,sizeof(issuedData->attrVals.wBitBuf));
    }
...
static int8_t ICACHE_FLASH_ATTR gizCheckReport(dataPoint_t *cur, dataPoint_t *last)
{
    ...
    if(last->valueLED0on_off != cur->valueLED0on_off)
    {
        GIZWITS_LOG("valueLED0on_off Changed\n");
        ret = 1;
    }
    if(last->valueLED1on_off != cur->valueLED1on_off)
    {
        GIZWITS_LOG("valueLED1on_off Changed\n");
        ret = 1;
    }
...
static int8_t ICACHE_FLASH_ATTR gizDataPoints2ReportData(dataPoint_t *dataPoints , devStatus_t *devStatusPtr)
{
    ...
    gizStandardCompressValue(LED0on_off_BYTEOFFSET,LED0on_off_BITOFFSET,LED0on_off_LEN,(uint8_t *)devStatusPtr,dataPoints->valueLED0on_off);
    gizStandardCompressValue(LED1on_off_BYTEOFFSET,LED1on_off_BITOFFSET,LED1on_off_LEN,(uint8_t *)devStatusPtr,dataPoints->valueLED1on_off);
